...
# on_change is needed because when Bob switches meal category (parent), we must
# immediately clear the child meal selection — a plain if-block can't clear
# session state before the widget is re-rendered, causing a stale mismatch.
def on_rate_category_change():
    st.session_state.rate_meal_sel = None

# ── Callback: on_click — Reset ratings form ─────────────────────────────────
# on_click is necessary to reset MULTIPLE form fields atomically (category,
# meal selection, star slider, visibility toggle) in one callback before the
# next render. A plain if-block would only run after widgets are drawn,
# leaving stale values visible for one frame.
...
# The rating form is hidden by default to keep the page clean. Bob can expand
# it when he wants to log a rating. This avoids cluttering the profile view
# with a form Bob may not need on every visit.
st.write("")
show_form = st.toggle(
    "⭐ Rate a Meal",
