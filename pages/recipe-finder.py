...
# on_click callback is necessary here because we need to reset MULTIPLE pieces
# of session state atomically before the next render — a plain if-block after
# the button press would fire a frame too late and leave stale widget values.
...
# on_change is needed here to clear the child dropdown selection whenever the
# parent category changes — if we used a plain if-block the stale child value
# would remain in session state and produce a misleading selection.
...
# The calorie slider, prep time filter, and chart only appear when the user
# enables "Advanced Filters" — keeps the UI clean for quick browsing but gives
# Bob deeper controls when he wants to optimise for his iron-deficiency diet.
st.write("")
...
# The specific-recipe dropdown only appears AFTER the user picks a meal
# category. Its options are filtered to that category, so Bob always sees
# relevant meals (e.g. only Breakfast recipes when planning breakfast).
